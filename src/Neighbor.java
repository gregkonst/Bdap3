...
public class Neighbor implements Comparable<Neighbor>{
    ...
    /**
     * Code changed here to sort from highest to lowest
     */
    public int compareTo(Neighbor nb) {
        if (Math.abs(this.similarity) < Math.abs(nb.getSimilarity())) {
            return 1;
        } else if (Math.abs(this.similarity) == Math.abs(nb.getSimilarity())){
            return 0;
        } else{
            return -1;
        }
